...
export function CaseStudyPage() {
  ...
  return (
    <article className="section-pad py-16">
      ...
      <h1 className="display-xl mt-4 text-6xl sm:text-8xl">{project.title}</h1>
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        {project.url && project.live !== false && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-full bg-gold px-5 py-2 font-mono text-xs uppercase tracking-widest text-void"
          >
            Live site
          </a>
        )}
        {project.github && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-full border border-white/20 px-5 py-2 font-mono text-xs uppercase tracking-widest"
          >
            GitHub
          </a>
        )}
      ...
      {project.thumbnail ? (
        <img
          ...
          className="mt-12 w-full rounded-[2rem] border border-white/10 object-cover"
        />
      ) : (
        <div
          className="mt-12 h-56 rounded-[2rem] border border-white/10"
          style={{ background: `linear-gradient(135deg, ${project.accent}, #111)` }}
        />
      )}
